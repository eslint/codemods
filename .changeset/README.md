...
When a PR is merged to `main`, the `release.yml` workflow consumes these files, bumps the relevant `package.json` and `codemod.yaml` versions, commits **Version Packages**, creates git tags, and publishes the affected codemods.

...
function splitAtFirstTopLevelDelimiter(src: string, delimiterChar: string): { before: string; after: string } | null {
  let braceDepth = 0
  let bracketDepth = 0
  let parenDepth = 0
...
function substituteBindingsOnce(trimmed: string, bindings: Map<string, string>, skipNames: Set<string>): string {
  if (!bindings.size) {
    return trimmed
  }
  const HEADER = 'void ('
  ...
  let ast: SgRoot<JS>
  try {
    ast = parse('javascript', wrappedSrc)
  } catch {
    return trimmed
  }
  ...
  for (const id of ids) {
    ...
    if (trimmed.slice(start, end) !== name) {
      continue
    }
...
function resolveRuleValueRaw(valueAfterColon: string, bindings: Map<string, string>, skipNames: Set<string>): string {
...
function parseStaticRequireRhs(rhsTrimmed: string): { specifier: string } | null {
  try {
    const wrapped = parse('javascript', `const __bind_req_probe = ${rhsTrimmed};`)
    ...
    if (callee?.kind() !== 'identifier' || callee.text() !== 'require') {
      return null
    }
...
function collectRequireImportsFromProgram(rootNode: SgNode<JS>): { binding: string; specifier: string }[] {
  const raw: { binding: string; specifier: string }[] = []
  const declarators = rootNode.findAll({
    rule: {
      kind: 'variable_declarator',
    },
  ...
  const out: { binding: string; specifier: string }[] = []
...
function specifierAlreadyInImports(imports: string[], specifier: string): boolean {
  const esc = specifier.replaceAll(/[.*+?^${}()|[\]\\]/g, '\\$&')
...
function resolveIdentifierInitializerText(idNode: SgNode<JS>, bindings: Map<string, string>): string | null {
  ...
  if (typeof idNode.definition !== 'function') {
    return null
  }
...
function resolveInitializerToObjectNode(
  ...
  const wrapped = parse('javascript', `const __spreadProbe = (${text});`)
  ...
  return (obj as SgNode<JS> | undefined) ?? null
}
...
function extractRulesFromObjectExpression(
  ...
  for (const member of members) {
    if (member.kind() === 'spread_element') {
      const spreadId = member.find({ rule: { kind: 'identifier' } })
      const name = spreadId?.text()
      if (!spreadId || !name) {
        continue
      }
      if (visitedSpreadVars.has(name)) {
        continue
      }
      ...
      const inner = extractRulesFromObjectExpression(spreadObj, bindings, visitedSpreadVars, skipImportBindings)
...
function getRulesValueExpression(rulesPair: SgNode<JS>): SgNode<JS> | null {
  const candidates = rulesPair
    ...
    .filter((n) => {
      const p = n.parent()
      return !!(p && hasSameRange(p, rulesPair))
    })
...
async function transform(root: SgRoot<JS>): Promise<string | null> {
  ...
  const staticRequireImportBindings = new Set(collectRequireImportsFromProgram(rootNode).map((r) => r.binding))
  ...
  for (let sector of rulesSectorsRule) {
    ...
    if (parserPairRule) {
      ...
      if (parserValueRule) {
        ...
        // Remove quotes
        if (
          (parserString.startsWith("'") && parserString.at(-1) === "'") ||
          (parserString.startsWith('"') && parserString.at(-1) === '"')
        ) {
    ...
    if (overridesRule) {
      ...
      const newSectorRoot = parse('javascript', newSectorText)
      sector = newSectorRoot.root() as unknown as SgNode<JS>
    }
    ...
    for (const jsdoc of requireJsdocRule) {
      ...
      if (identifier === 'rules') {
        const pair = jsdoc
          ...
          .replace('"require-jsdoc":', '')
          .trim()
        if ((pair?.[0] === '"' && pair.at(-1) === '"') || (pair?.[0] === "'" && pair.at(-1) === "'")) {
    ...
    if (pluginsRule) {
      ...
      if (pluginsObject) {
        ...
        for (const pluginPair of pluginPairs) {
          ...
          if (
            (pluginName.startsWith('"') && pluginName.at(-1) === '"') ||
            (pluginName.startsWith("'") && pluginName.at(-1) === "'")
          ) {
    ...
    if (noUnusedVarsRule) {
      ...
      if (isArrayRule.length) {
        ...
        for (const option of optionsRule) {
          const identifier = option.getMatch('IDENTIFIER')?.text()
          let value: string | number | boolean = option.text().trim().replace(`${identifier}:`, '').trim()
          if (!identifier) {
            continue
          }
    ...
    if (camelcaseRule) {
      ...
      if (isArrayRule.length) {
        ...
        for (const option of optionsRule) {
          ...
          let value: string | boolean | number = option.text().trim().replace(`${identifier}:`, '').trim()
          if (value === 'true' || value === 'false') {
            value = value === 'true'
    ...
    for (const noRestrictedImports of noRestrictedImportsRule) {
      ...
      if (paths.length) {
        ...
        pairs = pairs.filter((pair) => {
          ...
          if (pairText.startsWith('paths:') || pairText.startsWith("'paths':") || pairText.startsWith('"paths":')) {
            return false
          }
          return true
        })
    ...
    for (const glob of detectGlobalsRule) {
      ...
      let value: string | number | boolean = glob.text().trim().replace(`${identifier}:`, '').trim()
      if (value === 'true' || value === 'false') {
        value = value === 'true'
      } else if (!Number.isNaN(Number.parseInt(value))) {
        value = Number.parseInt(value)
      ...
      if (
        (identifier.startsWith("'") && identifier.at(-1) === "'") ||
        (identifier.startsWith('"') && identifier.at(-1) === '"')
      ) {
  ...
  for (const req of collectRequireImportsFromProgram(rootNode)) {
    if (specifierAlreadyInImports(imports, req.specifier)) {
      continue
    }
